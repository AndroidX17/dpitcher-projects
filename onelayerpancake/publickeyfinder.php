...
$additionalwalletinfo = $walletinfo['address'];
...
//  echo "<BR>";
 ...
 // echo "<BR>";
  ...
  $totalmoneysent = $moneysent;
  $totalmoneygained = $moneygained;
  $totalincomingtx = $chainstats['funded_txo_count'];
  $totaloutgoingtx = $chainstats['spent_txo_count'];
...
$nextqueryallowed = 1;
  ...
  if ($chainstats['spent_txo_count'] === 0) {

echo "This wallet has never spent money, so it has not yet unveiled its public key to us all.";


$nextqueryallowed = 0;
if ($chainstats['tx_count'] === 0) {

    echo "This wallet has never received money either, so you might call it:";
    echo "<BR> a fresh wallet with no transactional history<BR>";
...
$append = $lasthash;
  ...
  $url2 = "https://mempool.space/api/address/".$wallet."/txs/chain/"; // chain?last_seen_txid=25 ... ????
  $parameters2 = [];
...
/*
  ...
  $totalgained = 0;
  $totalsent = 0;
  $dateourguygaveuppublickey = "";
  //echo " YOU LOOKED UP THIS WALLET: ".$wallet."<BR>";
 // echo "<BR>";
  foreach($txinfo as $txn) // this is all of the transactions
  {
...
$currentdateknown = date(DATE_RFC2822,  $timestamp);
  
  //echo "The date of transaction: ".date('l jS \of F Y h:i:s A',  $timestamp); // m/d/Y
  
  
  
//  echo "<BR>--------------------"; // OP_0  single signature??
    /*
  if($txn && isset($txn['witness']) && $txn['witness']) // we are looking at the array of all of the addresses on the left side of mempool
  {


    echo "WITNESS DETECTED<BR>";

  }
  */
  
  if($txn && isset($txn['vin']) && $txn['vin']) // we are looking at the array of all of the addresses on the left side of mempool
  {
  ...
  $totalshownleftside = 0;
    ...
    foreach($incomingthings as $incomingthing) { // this has 600 entries
...
$newpublickeyfound = "";
        ...
        if($incomingthing && isset($incomingthing['scriptsig']) && $incomingthing['scriptsig'])
        {
      
      $scriptsig = $incomingthing['scriptsig'];

    //  echo "<BR><BR>We have made it to level 2: ".$scriptsig;
        ...
        if($incomingthing && isset($incomingthing['witness']) && $incomingthing['witness'])
        {
      
      $witness = $incomingthing['witness'];

     // echo "<BR><BR>We have made it to level 3: ".$witness;

   //  echo "SIGNATURE FOUND:".$witness['0']."<BR>"; 
   //  echo "PUBLIC KEY FOUND:".$witness['1']."<BR>"; 

   $dateourguygaveuppublickey = $currentdateknown;
     $newsignaturefound = $witness['0'];
     $newpublickeyfound = $witness['1'];
     /*
   foreach ($witness as $thingInWitness) {


    echo "thing in witness: ".$thingInWitness;

   }*/
/* broken
   echo "SIGNATURE FOUND:".$thingInWitness[0]."<BR>"; // 1600142e91d40417f877b5a9f20ebec843bcf4f3c51356
   echo "PUBLIC KEY FOUND:".$thingInWitness[1]."<BR>";
 ...
 // here we sift through the wheelbarrow of 600 for our guy's wallet

// this is where we can handle multiple entries
 $whatwastheSIGNATURE =  $whatwastheSIGNATURE."///".$newsignaturefound;
     $whatwasthepUBLICKEY = $whatwasthepUBLICKEY."///".$newpublickeyfound;
...
/*
      ...
      echo "<BR> here is scriptpubkey ".$incomingthing['scriptpubkey'];
      echo "<BR> here is scriptpubkey_asm ".$incomingthing['scriptpubkey_asm'];
      echo "<BR>";
      echo "<BR>";
...
echo "<DIV class='publickeyfound' style=''>";
echo "<DIV style=''>";
echo " HERE ARE THE RESULTS OF THE SEARCH -  <BR><BR>";


  

echo "SIGNATURE:".$whatwastheSIGNATURE."<BR><BR>";
echo "public key:".$whatwasthepUBLICKEY."<BR><BR>";
echo "<BR><BR><BR>";

echo "BALANCE: ".$currentbalance." btc <BR><BR>";


echo "<U>ADDITIONAL WALLET INFO</U><BR><BR>  for  ".$additionalwalletinfo."<BR><BR>";
echo "money sent: ".$totalmoneysent." btc <BR><BR>";
echo "money gained: ".$totalmoneygained." btc <BR><BR>";
echo "incoming tx count: ".$totalincomingtx."<BR><BR>";
echo "outgoing tx count: ".$totaloutgoingtx."<BR><BR>";
...
if (isset($_SESSION["useruid"])) {
  ...
  if ($_SESSION["permissions"] === "SPECIAL") {
    ...
    include  __DIR__ . '/templates/publickeyfinder.html.php';
